#include "threadPool.h" // 引入自定义的线程池头文件
...
{
    // ./server 192.168.102.128 2333 3
    ARGS_CHECK(argc,4); // 检查命令行参数个数是否正确，应该为4个

    threadPool_t threadPool; // 定义线程池结构体
    int workerNum = atoi(argv[3]); // 将第三个命令行参数转换为整型，表示工作线程数量
    initThreadPool(&threadPool,workerNum); // 初始化线程池

    int sockfd; // 定义套接字描述符
    tcpInit(argv[1],argv[2],&sockfd); // 初始化TCP连接，将服务器IP和端口号作为参数传入，并将返回的套接字描述符保存在sockfd中
